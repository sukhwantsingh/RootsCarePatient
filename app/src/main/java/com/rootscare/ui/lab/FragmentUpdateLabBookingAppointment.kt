...
import com.rootscare.ui.lab.adapter.AdapterExtraLabHospital
...
class FragmentUpdateLabBookingAppointment :
    BaseFragment<FragmentBookingLabBinding, FragmentUpdateLabBookingAppointmentViewModel>(),
    FragmentUpdateLabBookingAppointmentNavigator, AdapterExtraLabHospital.AdapterCallback ,
    GetSlotTimingInterface {
    private var fragmentNursesBookingAppointmentBinding: FragmentBookingLabBinding? = null
    private var fragmentNursesBookingAppointmentViewModel: FragmentUpdateLabBookingAppointmentViewModel? =
        null
...
//    private var labPrice: String? = ""
    ...
    var nurseDetailsResponseData: HospitalDetailsResponse?=null
    var labArr =
        arrayListOf<com.rootscare.data.model.api.response.hospitalallapiresponse.hospitallabresponse.ResultItem?>()
    lateinit var contactListAdapter: AdapterExtraLabHospital
    override val bindingVariable: Int
        get() = BR.viewModel
    override val layoutId: Int
        get() = R.layout.fragment_booking_lab
    override val viewModel: FragmentUpdateLabBookingAppointmentViewModel
        get() {
            fragmentNursesBookingAppointmentViewModel =
                ViewModelProviders.of(this)
                    .get(FragmentUpdateLabBookingAppointmentViewModel::class.java)
            return fragmentNursesBookingAppointmentViewModel as FragmentUpdateLabBookingAppointmentViewModel
        }
...
//            if (nurseVisitPrice != "0") {
            if (fromBookingTime != "" && toBookingTime != "") {
                ...
                val date1 =
                    dateFormat.parse(selectedstartTime/*fragmentNursesBookingAppointmentBinding?.edtNurseFromTime?.text.toString()*/)
                val date2 =
...
//                        println("Compare Result 4 : " + calendar2.compareTo(calendar1))
                ...
                if (calendar3.compareTo(calendar1) == 1 || calendar4.compareTo(calendar2) == -1) {
                    Toast.makeText(
                        activity,
                        "Lab operating time is from $nurseStartTime to $nurseEndTime. Please choose in between time.",
                        Toast.LENGTH_SHORT
                    ).show()
...
//        val contactListAdapter = AdapterHospitalRecyclerviw(trainerList,context!!)
    ...
    fun apiHitGetBookedSlots() {
        if (isNetworkConnected) {
            ...
            getSlotRequest.service_type = "pathology"
    ...
    private fun apiHitForNurseTask() {
        ...
        if (isNetworkConnected) {
            baseActivity?.showLoading()
            val nurseHourlySlotRequest = DoctorRequest()
//            nurseHourlySlotRequest.hospital_id = "114"
            nurseHourlySlotRequest.hospital_id = nurseId.toInt().toString()
    ...
    // Set up recycler view for service listing if available
    private fun populateRecyclerView(labList: ArrayList<com.rootscare.data.model.api.response.hospitalallapiresponse.hospitallabresponse.ResultItem?>?) {
        labArr.clear()
        taskId = ""
        task = StringBuilder()
        ...
        contactListAdapter = AdapterExtraLabHospital(
            labArr,
            activity,
            this@FragmentUpdateLabBookingAppointment
...
//                fragmentNursesBookingAppointmentBinding?.txt
    ...
    override fun successNurseTaskResponse(nurseDetailsResponse: HospitalLabResponse?) {
        if (nurseDetailsResponse?.code.equals("200")) {

            if (nurseDetailsResponse?.result?.lab != null && nurseDetailsResponse.result.lab.size > 0) {
                fragmentNursesBookingAppointmentBinding?.llHourlyTime?.visibility = View.GONE
                fragmentNursesBookingAppointmentBinding?.llHourlyTimeNew?.visibility = View.VISIBLE
                // fragmentNursesBookingAppointmentBinding?.recyclerViewRootscareFromTimeRecyclerview?.visibility=View.VISIBLE
                //  fragmentNursesBookingAppointmentBinding?.tvNoDateSlottime?.visibility=View.GONE
                populateRecyclerView(nurseDetailsResponse.result.lab)
            } else {
                fragmentNursesBookingAppointmentBinding?.llHourlyTime?.visibility = View.GONE
...
//        fragmentNursesBookingAppointmentBinding?.rlHourlyPrice?.visibility=View.VISIBLE
        fragmentNursesBookingAppointmentBinding?.recyclerViewRootscareFromTimeRecyclerview?.visibility =
            View.VISIBLE
        fragmentNursesBookingAppointmentBinding?.tvNoDateSlottime?.visibility = View.GONE
        fragmentNursesBookingAppointmentBinding?.recyclerViewLabTest1Type?.visibility = View.VISIBLE
        fragmentNursesBookingAppointmentBinding?.recyclerViewRootscareHourlyTimeRecyclerview?.visibility =
            View.VISIBLE
        if (isNetworkConnected) {
            baseActivity?.showLoading()
            ...
            val nurseHourlySlotRequest1 = DoctorRequest()
            nurseHourlySlotRequest1.hospital_id = "39"
...
//            familymemberid.toRequestBody("multipart/form-data".toMediaTypeOrNull())
        ...
        val symptom_text =
            fragmentNursesBookingAppointmentBinding?.edtSymptomText?.text?.toString()!!
                .toRequestBody("multipart/form-data".toMediaTypeOrNull())
        val task_id = taskId.toString().toRequestBody("multipart/form-data".toMediaTypeOrNull())
...
fun getSlots( fisrt_date : String ,second_date : String , starttime: String , endtime: String,diff:Long,timetoadd:Int){
    ...
    slotsList?.clear()
    FragmentNurseAppointmentReschedule.mySlotListTemp?.clear()
    ...
    while (dif < dateObj2.time) {
        ...
        if (sdff.parse(ViewUtils.getCurrentDate())==sdff.parse(fisrt_date))
        {
            ...
            if (System.currentTimeMillis() > strDate.time) {
                ...
                FragmentPhysiotherapyBookingAppointment.mySlotListTemp.add(SlotModel(false,formateDate,false,timetoadd))
            }else{

                slotsList?.add(SlotModel(false,formateDate,true,timetoadd))
                FragmentPhysiotherapyBookingAppointment.mySlotListTemp.add(SlotModel(false,formateDate,true,timetoadd))
            }}
        else
        {
            slotsList?.add(SlotModel(false,formateDate,true,timetoadd))
