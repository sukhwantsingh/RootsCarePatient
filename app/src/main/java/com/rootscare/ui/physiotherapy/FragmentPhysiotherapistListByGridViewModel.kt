...
class FragmentPhysiotherapistListByGridViewModel :
    BaseViewModel<FragmentPhysiotherapistListByGridNavigator>() {
    fun apinurselist() {
//        val body = RequestBody.create(MediaType.parse("application/json"), "")
    ...
    fun apiPhysiotherapyList() {
        val disposable = apiServiceWithGsonFactory.apiPhysiotherapyList()
            .subscribeOn(_scheduler_io)
            .observeOn(_scheduler_ui)
            .subscribe({ response ->
                if (response != null) {
...
//        val body = RequestBody.create(MediaType.parse("application/json"), "")
        val disposable =
            apiServiceWithGsonFactory.apidepartmentnurselist(departmentNurseListRequest)
                .subscribeOn(_scheduler_io)
                .observeOn(_scheduler_ui)
                .subscribe({ response ->
                    if (response != null) {
