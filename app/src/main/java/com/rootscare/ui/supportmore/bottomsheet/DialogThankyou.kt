...
import com.rootscare.databinding.LayoutBsThankyouSuccessBinding
...
class DialogThankyou : DialogFragment() {
    ...
    companion object {
      ...
      private var mCallback: OnBottomSheetCallback? = null

        fun newInstance(clb: OnBottomSheetCallback? = null, vararg data: String?): DialogThankyou {
            data.getOrNull(0)?.let{ mMsg = it }
            data.getOrNull(1)?.let{ mBtnText = it }
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        ...
        try {
            binding.run {
             mMsg?.let { mg -> tvsh2.text = mg }
             mBtnText?.let { mbt -> tvClose.text = mbt   }
