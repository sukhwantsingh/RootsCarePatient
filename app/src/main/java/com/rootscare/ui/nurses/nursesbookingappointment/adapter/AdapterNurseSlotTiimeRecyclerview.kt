...
class AdapterNurseSlotTiimeRecyclerview(
    val nurseTimingList: ArrayList<ResultItem?>?,
    internal var context: Context
) : RecyclerView.Adapter<AdapterNurseSlotTiimeRecyclerview.ViewHolder>() {
    companion object {
...
//
            ...
            itemView.root.ll_nurse_slot_time?.setOnClickListener {
                ...
                recyclerViewItemClickWithView.onConfirm(nurseTimingList?.get(localPosition)!!)
            }
        }
        ...
        fun onBind(pos: Int) {
            ...
            itemView.rootView?.txt_nurse_view_timings?.text = nurseTimingList?.get(pos)?.startTime

            if (nurseTimingList?.get(pos)?.status.equals("Active")) {
                itemView.rootView?.txt_nurse_view_timings?.alpha = 1.0F
                itemView.rootView?.ll_nurse_slot_time?.isClickable = true
            } else if (nurseTimingList?.get(pos)?.status.equals("Inactive")) {
                itemView.rootView?.txt_nurse_view_timings?.alpha = 0.4F
