...
class VideoCallActivity : AppCompatActivity() {
    ...
    */
    ...
    private var speakerActionFab: FloatingActionButton? = null
...
//    var alertDialog: AlertDialog? = null
    ...
    private fun readFromBundle() {
        ...
        println("isDoctorCalling $isDoctorCalling")
    }
    ...
    private val statusOfCall: Unit
        get() {
            ...
            */
            ...
            clientOwnName = appSharedPref?.userId?.replace("\\s", "")
            ...
            */
            if (checkPermissionForCameraAndMicrophone()) {
                ...
                callComingFromOutside()
            }
        }

    private fun callComingFromOutside(){
        if (isDoctorCalling!!) {
            if (!roomName.equals("", ignoreCase = true)) {
...
//        chronometer!!.visibility = View.GONE
    ...
    private fun showVideoIncomingDialog(name: String?) {
        alertDialog = name?.let {
            Dialog.createStudentIncomingCallDialog(
                answerVideoCallClickListener(),
                cancelVideoCallClickListener(),
                this,
                it
            )
        }
    ...
    private val mInfoReceiver: BroadcastReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent) {
            if (intent.action == Intent.ACTION_HEADSET_PLUG) {
                println("musicIntentReceiver BroadcastReceiver ----------")
                ...
                if (!CommonUtils.isCharging(this@VideoCallActivity)) {
                    if (level <= 10) {
                        Snackbar.make(clVideo!!,"Your battery level is low. Please plug into charger to continue", Snackbar.LENGTH_SHORT).show()
                    }
                    if (level <= 5) {
                        if (room != null) {
    ...
    private fun cancelVideoCallClickListener(): DialogInterface.OnClickListener {
        return DialogInterface.OnClickListener { _: DialogInterface?, _: Int ->
            isCallTaken = true
            callRejectPushNotification("Patient $toUserName rejects the call.")
        }
    }
...
//            connectActionFab!!.isEnabled = false
            println("disconnectClickListener $room")
            isCallTaken = false
            /*
             * Disconnect from room
             */
//            println("localParticipant.getVideoTracks().size() " + room.getParticipants().size());

//            System.out.println("localParticipant.getVideoTracks().size() " + room.getParticipants().size());
            println("room.getRemoteParticipants().size() " + room!!.remoteParticipants.size)
            if (room != null) {
                if (room?.remoteParticipants?.size == 1) {
                    println("test  if$isCallTaken")
                    if (timeCounter > 60) {
                        println("test time_counter if")
                        disconnectCall()
                    } else {
                        println("test time_counter else")
                        room?.disconnect()
                        chronometer?.stop()
                        timer?.cancel()
                        timer1?.cancel()
                    ...
                    callRejectPushNotification("Patient $toUserName rejects the call.")
                    println("callRejectPushNotification")
//                  if (!isCallTaken && !isTeacherCalling)
    ...
    override fun onResume() {
        super.onResume()
        this.registerReceiver(mInfoReceiver,
            IntentFilter().apply {
        ...
        if (localVideoTrack == null && requestPermissions() && cameraCaptureCompat != null) {
             ...
             */
            println("localParticipant $localParticipant")
...
//        chronometer.start();
    ...
    override fun onPause() {
        ...
        super.onPause()
      this.unregisterReceiver(mInfoReceiver)
    }

    override fun onDestroy() {
        println("hello onDestroy")
        /*
...
//                    println("minute_mod $minuteMod")
                    println("time_counter $timeCounter")
                }
            }
        }, 1000, 1000)
    }
    ...
    private fun createAudioAndVideoTracks() {
        ...
        localVideoTrack = cameraCaptureCompat?.let { LocalVideoTrack.create(this, true, it) }
...
//        retrieveAccessTokenFromServer()
        ...
        twilioAccessTokenRequest.identity = appSharedPref?.userId
        twilioAccessTokenRequest.roomName = roomName
        println("twilioAccessTokenRequest $twilioAccessTokenRequest")
...
//        videoStatusTextView.setText("Participant " + participant.getIdentity() + " left.");
    ...
    private fun moveLocalVideoToPrimaryView() {
        if (thumbnailVideoView?.visibility == View.VISIBLE) {
            ...
            localVideoView = primaryVideoView
            primaryVideoView?.mirror = cameraCaptureCompat?.cameraSource == CameraCaptureCompat.Source.FRONT_CAMERA
        }
    }
    ...
    private fun roomListener(): Room.Listener {
        return object : Room.Listener {
            ...
            override fun onConnected(room: Room) {
                println("onConnected")
                println("room getLocalParticipant " + room.localParticipant)
                println("room id " + room.sid)
//                if (isTeacherCalling) {
...
//                }
                ...
                title = room.name
                println("room getParticipants " + room.remoteParticipants.size)
...
//                videoStatusTextView.setText("Disconnected from " + room.name)
            ...
            override fun onParticipantConnected(room: Room, remoteParticipant: RemoteParticipant) {
                println("onParticipantConnected")
                isCallTaken = true
            ...
            override fun onParticipantDisconnected(
                ...
                println("onParticipantDisconnected")
    ...
    private fun mediaListener(): RemoteParticipant.Listener {
        return object : RemoteParticipant.Listener {
            ...
            override fun onAudioTrackUnpublished(
                ...
                println("onAudioTrackUnpublished")
            }
            ...
            override fun onVideoTrackUnpublished(
                ...
                println("onVideoTrackUnpublished")
            ...
            override fun onDataTrackUnpublished(
                ...
                println("onDataTrackUnpublished")
            }
            ...
            override fun onAudioTrackSubscribed(
                ...
                println("onAudioTrackSubscribed")
                if (timer1 != null)
                    timer1!!.cancel()
                calculateFee()
            }
            ...
            override fun onAudioTrackUnsubscribed(
                ...
                println("onAudioTrackUnsubscribed")
            }
            ...
            override fun onAudioTrackSubscriptionFailed(
                ...
                println("onAudioTrackSubscriptionFailed")
            }
            ...
            override fun onVideoTrackSubscribed(
                ...
                println("onVideoTrackSubscribed")
                rl01!!.visibility = View.GONE
            ...
            override fun onVideoTrackUnsubscribed(
                ...
                println("onVideoTrackUnsubscribed")
                if (thumbnailVideoView != null) {
            ...
            override fun onVideoTrackSubscriptionFailed(
                ...
                println("onVideoTrackSubscriptionFailed")
                Snackbar.make(
                    clVideo!!,
                    String.format(
            ...
            override fun onDataTrackUnsubscribed(
                ...
                println("onDataTrackUnsubscribed")
            }
            ...
            override fun onDataTrackSubscriptionFailed(
                ...
                println("onDataTrackSubscriptionFailed")
            }
    ...
    private fun getAccessToken(twilioAccessTokenRequest: TwilioAccessTokenRequest) {
        val disposable =
            ApplicationClass.instance!!.apiServiceWithGsonFactory.getAccessTokenForVideo(twilioAccessTokenRequest)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe({ response ->
                    if (response != null) {
                        // Store last login time
                        Log.e("check_response", ": " + Gson().toJson(response))
                        accessToken = response.result
                        if (accessToken != null) connectToRoom()
                        println("accessToken $accessToken")
                    } else {
                        Log.e("check_response", ": null response")
                    }
                }, { throwable ->
                    run {
                        Log.e("check_response_error", ": " + throwable.message)
                    }
                })
    ...
    private fun callRejectPushNotification(messageBody: String) {
        ...
        videoPushRequest.type = "patient_to_doctor_video_call_reject"
    ...
    private fun sendPushNotification(videoPushRequest: VideoPushRequest) {
        val disposable = ApplicationClass.instance?.apiServiceWithGsonFactory?.sendVideoPushNotification(videoPushRequest)
                ?.subscribeOn(Schedulers.io())
                ?.observeOn(AndroidSchedulers.mainThread())
                ?.subscribe({ response ->
                    if (response != null) {
